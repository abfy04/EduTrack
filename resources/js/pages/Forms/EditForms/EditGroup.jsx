...
export default function EditGroup() {
  ...
  const { group, filieres = [], years = [], options = [], errors: inertiaErrors } = usePage().props;
  ...
  const { data, setData, put, processing } = useForm({
    libel: group?.name || '',
    filiere: group?.parent?.parent?.id || '', // Assuming group -> year -> filiere hierarchy
    year: group?.parent?.id || '',
    option: group?.id || ''
  });
  ...
  const isSubmitDisabled = () => {
    return !data.libel || !data.filiere || !data.year || !data.option || localErrors.libel || processing;
  };
